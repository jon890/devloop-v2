...
미래의 소비자인 AI 에이전트가 묻는 방식(EVAL-RUBRIC 섹션 2의 A1~A5 패턴)으로 판정한다.
정형 질의는 anchor 모호성이 없으므로 미달은 곧 그래프·질의 엔진의 결함이다 — 특히 A(앵커) 미달을 앵커 모호성으로 변명할 수 없다.
...
2. **ground truth 대조** — 질문에 `groundTruthCypher` 가 있으면(집계·목록형) 그 쿼리를 직접 실행한 결과와 답변을 대조한다.
   불일치는 **R 축**으로 판정한다 (EVAL-RUBRIC 섹션 3 "R 축 상세" — ground truth 결과 행 대비 일치율).
   gold 가 함께 있으면 gold 는 evidence 확인용 보조로만 쓰고, 판정 주체는 ground truth 다.
   집계 결과가 틀렸는데 관계 주장도 함께 틀렸으면 G 도 별도로 미달 판정한다 — 두 축은 독립이다.
3. **근거 id 검사** — A4 패턴 질문은 응답 evidence 의 노드 id 가 실제 그래프에 존재하는지 표본 확인한다 (P 축).
4. 리포트: `eval/reports/<YYYY-MM-DD>-ai-<stage>.md`, 판정은 EVAL-RUBRIC 섹션 4 통과 조건 (축별 독립 판정).
