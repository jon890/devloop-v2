...
| `baseRevision` | string | 실행 기준 revision |
| `validationCommand` | string[] | 실행 조건으로 고정한 검증 명령 |
...
이어 쓰기는 `suiteHash`, `sourceLockHash`, `baseRevision`, `validationCommand`, `memoryIndexHash`가 모두 같을 때만 허용한다.
