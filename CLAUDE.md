...
### 검색이 닿지 않는 곳이 있다 (2026-08-03 실측)

`FULLTEXT_INDEXES` 가 **제목과 이름만** 덮는다. 본문과 댓글에는 인덱스가 없다.

그런데 그래프에 저장되는 텍스트 자체가 머리 부분뿐이다 — 업무 본문 300자, 댓글 200자
(`structural-extractor.ts:61`·`:307`). 원천 중앙값이 414자와 495자이므로 댓글의 79%가 잘린다.

- 실측 — 어떤 질문이 병목과 조치를 함께 묻는데, 정답 댓글에서 병목은 앞 200자에 있고 **조치는 그 뒤라 그래프에 없다.**
  검색이 찾아내더라도 인용할 내용이 없다
- 즉 `GRAPH` 관문이 0건이라고 해서 "그래프에 답이 다 있다" 는 뜻이 아니다. 그 관문은 노드·관계의 존재만 보고 본문이 온전한지는 보지 않는다
...
- 현재 개수 — api 51, pipeline 140 (2026-07-31 실측). 줄었으면 무언가 실행되지 않는 것이다
