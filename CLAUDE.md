...
### 총수가 안 움직이면 실패를 분해해 다음 원인을 찾는다 (2026-08-03)

근거 예산만 바꿨을 때 회수 실패가 12에서 12로 총수가 같았다. 여기서 멈추지 않고 남은 실패 19건을
갈랐다 — "부모 업무는 근거에 왔는데 그 근거만 빠졌다" 12건, "부모 업무에 도달 못 함" 7건이다.

앞쪽 8회의 Cypher 를 직접 열어 원인을 찾았다. **`LIMIT 50` 이라고 짐작했으나 아니었다.**

| 관측 | 회차 |
| --- | ---: |
| 지목된 업무 중 **하나만** 댓글을 확장했다 | 5 |
| `HAS_COMMENT` 를 아예 쓰지 않았다 | 3 |

Cypher 생성 프롬프트에 "지목된 모든 업무를 number 목록에 넣어 댓글을 함께 확장하라" 를 넣자
회수 실패가 (12, 12) 에서 (7, 9) 로 떨어졌다. **구간이 겹치지 않는다 — 이 축에서 처음이다.**

- **같은 코드로 두 번 재야 구간이 생긴다.** 한 번만 재면 7이 효과인지 변동인지 알 수 없다
- **개선이 의도한 기제 때문인지 Cypher 로 확인하라.** `AGW-A02` 가 4→0 이 됐고 3회 전부
  `WHERE t.number IN [497, 499]` 로 두 업무의 댓글을 함께 확장했다. 프롬프트에 넣은 패턴 그대로다
- **프롬프트 지시는 테스트로 고정하라.** 이 저장소의 Cypher 프롬프트는 어느 테스트도 문구를
  검증하지 않아 조용히 사라질 수 있었다. `llmCli` 대역으로 프롬프트를 포착한다
...
- 현재 개수 — api 70, pipeline 163 (2026-08-03 실측). 줄었으면 무언가 실행되지 않는 것이다
