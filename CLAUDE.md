...
- 현재 개수 — api 14, pipeline 35. 줄었으면 무언가 실행되지 않는 것이다
...
| 앵커 원문 보존·라벨 정원 | 리뷰 반영 중. 개행 오변환 1건이 마지막 차단 항목 |
| `query` 352줄 분해 | 예정. 앵커 브랜치 머지 후 착수 |
| prettier 도입 | 예정 |
...
앵커 브랜치 34문항 3회 측정에서 남은 실패 12건을 보면 같은 노드를 여러 문항이 함께 놓친다.
...
문항별로 대응하지 말고 이 노드들에 왜 도달하지 못하는지 보는 것이 효율적이다.
