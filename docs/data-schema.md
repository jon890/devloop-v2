...
| 대상 | 상한 | 전처리 |
| --- | --- | --- |
| `Task.bodyExcerpt` | 6,000자 | 없다 |
| `Comment.excerpt` | 6,000자 | GitHub 훅 머리말을 벗긴다 |

상한 6,000자는 평가 세트의 필수 근거 댓글 14건을 **전부** 담는 최소값이다
(길이 470~5,061자). 상한을 넘으면 잘리고, `parse-structure` 요약이 잘린 건수를 출력한다.

**훅 머리말을 벗기되 댓글 자체를 버리지 않는다.** 훅 댓글 359건은 예외 없이 커밋 메시지를
달고 있고, 평가 세트가 필수 근거로 지목한 것도 그중 2건이다. 잡음은 댓글이 아니라 매번 같은
모양으로 반복되는 링크 머리말이며 그 분량이 전체의 60%다.
...
"앞쪽 링크 줄을 걷어낸다" 는 넓은 규칙은 쓰지 않는다 — 사람이 멘션이나 불릿으로 시작한 댓글
317건이 함께 깎이는 것을 실측으로 확인했다. 좁힌 규칙은 훅 359건을 전부 잡고 사람 댓글은
한 건도 건드리지 않는다.

벗긴 뒤 남는 내용이 없으면 원문을 유지한다 — 정보를 잃지 않게 하는 안전 기본값이다.

머리말 벗기기는 저장하는 값에만 적용한다. 업무 참조 추출은 원문 전체를 그대로 받는다.
같은 텍스트를 미리 가공해서 넘기면 `REFERENCES` 328건이 조용히 바뀐다.
