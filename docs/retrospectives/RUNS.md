...
| 2026-08-14 | build-with-teams | plan016-memory-graph-spike | HIGH_RISK | 3 | 1 | 4 | 2 | 없음 | 0 | 관계형·일반 task 각 3회 memory-graph를 plan014 oracle-memory와 비교했다. 모든 oracle-memory와 memory-graph run이 성공했고 Graph LLM/direct call 0이었다. 관계 증거는 MEM-EXP-001 1/3만 관측되어 task별·전체 NO_ADDED_VALUE로 닫았다 |
| 2026-08-12 | build-with-teams | plan017-memory-automatic-eval | HIGH_RISK | 3 | 0 | 0 | 0 | 없음 | 0 | plan014 agent-triggered 12회와 plan017 automatic 12회 identity를 검증했다. EXP-001만 voluntary 2/3 성공+miss에서 automatic 3/3 성공·wrong edit 0으로 회수됐고, code-only automatic 조회 6회는 unnecessary로 분리했다. stale 0·success regression 0이지만 MEM-CODE-001 voluntary token sample이 3/3 finite가 아니라 INCONCLUSIVE로 닫았다 |
