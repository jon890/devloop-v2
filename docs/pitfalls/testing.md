...
- **현재 개수 — api 83 · pipeline 160 · @devloop/llm 19** (2026-08-05 실측). 줄었으면 무언가 실행되지 않는 것이다
- pipeline 이 163 에서 160 이 된 것은 plan010 에서 `buildCodexArgs` 테스트 3건을 함께 지운 결과다
- `@devloop/llm` 19건은 **루트에 집계 스크립트가 없어** `pnpm --filter @devloop/llm test` 를 손으로
  불러야 돈다. 안 부르면 이 19건이 조용히 빠진다
