...
`LLM_MODEL` 을 스키마 필수 값으로 만들지 마라. 모델이 필요 없는 단계도 같은 설정 스키마로
기동하기 때문이다. 다만 `infer-knowledge` 와 두 어댑터는 현재처럼 **사용 시점에 모델이 없으면
예외를 던져야 한다.** Codex 의 `-m` 과 Claude 의 `--model` 플래그도 모델이 있으면 항상 붙인다.
...
- Codex·Claude 어댑터와 `main.ts` 의 `infer-knowledge` 분기가 모델이 없으면 예외를 던지는지
  각각 단언한다. 모델이 있으면 Codex 의 `-m` 과 Claude 의 `--model` 플래그가 항상 붙어야 한다
- reasoning effort 가 없으면 Codex 의 `-c model_reasoning_effort=...` 플래그를 붙이지 않는지 단언한다
...
- 어댑터의 모델 필수 가드를 무력화 → 모델 부재 거부 테스트가 실패하는지
...
- **스키마 필수화를 더 하지 않는 이유** — 모델이 필요 없는 단계는 `LLM_MODEL` 없이도 기동한다.
  `infer-knowledge` 와 어댑터가 사용 시점에 모델 부재를 거부하는 기존 경계만 유지한다
