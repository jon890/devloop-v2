...
1. 기존대로 `inferred.jsonl` 과 리포트 파일을 읽는다
2. 파싱한 레코드와 누적 dropped 를 `sanitizeLlmRecords` 에 넘긴다
...
새 파일을 만들면 glob 도 갱신해야 실행된다. 통과 표시가 아니라 **개수**를 확인하라.
...
```bash
# cwd: 저장소 루트
cp apps/pipeline/data/graph/ocr-project/inferred.jsonl /tmp/inferred-before.jsonl
cp apps/pipeline/data/graph/ocr-project/inference-dropped-relationships.json /tmp/dropped-before.json
# 파일 함수를 직접 호출하는 최소 스크립트로 sanitizeLlmGraphFile 을 1회 실행한다
cmp /tmp/inferred-before.jsonl apps/pipeline/data/graph/ocr-project/inferred.jsonl
cmp /tmp/dropped-before.json apps/pipeline/data/graph/ocr-project/inference-dropped-relationships.json
```

프로젝트 코드는 로컬 데이터 디렉터리 이름을 확인해 맞춰라 (`apps/pipeline/data/graph/` 아래).
