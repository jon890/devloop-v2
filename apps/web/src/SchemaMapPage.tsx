    ...
    <section className="schema-map-page page-surface">
      <div className="map-heading">
        ...
        <div className="map-totals">
          <span><strong>{stats ? sum(stats.nodes).toLocaleString('ko-KR') : '—'}</strong>전체 노드</span>
          <span><strong>{stats ? sum(stats.relationships).toLocaleString('ko-KR') : '—'}</strong>전체 관계</span>
        </div>
      </div>
      ...
      {ontology && stats && (
        <>
          <div className="schema-workspace">
            ...
            <aside className="schema-sample-panel">
              ...
              <div className="sample-list" aria-live="polite">
                  ...
                  <ul>
                    {samples.nodes.map((node) => (
                      <li key={node.id}>
                        ...
                        <span><strong>{node.display}</strong><small>{node.key}</small></span>
                      </li>
                    ))}
                  </ul>
                )}
                    ...
                    {samples.relationships.map((relationship) => {
                      ...
                      return (
                        <li key={relationship.id}>
                          <span><strong>{start?.display ?? relationship.startId}</strong><small>{start?.label}</small></span>
                          <b aria-hidden="true">→</b>
                          <span><strong>{end?.display ?? relationship.endId}</strong><small>{end?.label}</small></span>
                        </li>
                      );
                    })}
                  </ul>
                ...
                {!sampleLoading
                  && ((selection.kind === 'label' && !samples.nodes.length)
                    || (selection.kind === 'relationship' && !samples.relationships.length))
          ...
          <div className="schema-type-index">
            <section>
              <div className="type-index-heading"><span>노드 라벨</span><small>{ontology.nodes.length}종</small></div>
            ...
            <section>
              <div className="type-index-heading"><span>관계 유형</span><small>{ontology.relationships.length}종</small></div>
