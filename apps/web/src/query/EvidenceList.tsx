...
export function EvidenceList({ evidence, onFocus, focusedId }: { evidence: Evidence; onFocus: (nodeId: string) => void; focusedId: string | null }) {
  return (
    <section className="evidence-section">
      ...
      <ol>
        {evidence.nodes.map((node, index) => (
          <li key={node.id}>
            <button type="button" className={focusedId === node.id ? "active" : ""} onClick={() => onFocus(node.id)}>
