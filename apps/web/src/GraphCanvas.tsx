...
export function GraphCanvas({ nodes, relationships, evidenceIds, focusedNodeId, onNodeClick, includeIsolatedNodes = false }: GraphCanvasProps) {
  ...
  useEffect(() => {
    ...
    const visibleRelationships = relationships.filter((relationship) => nodeIds.has(relationship.startId) && nodeIds.has(relationship.endId));
    const connectedNodeIds = new Set(visibleRelationships.flatMap((relationship) => [relationship.startId, relationship.endId]));
    const visibleNodes = includeIsolatedNodes ? nodes : nodes.filter((node) => connectedNodeIds.has(node.id));
    const elements: ElementDefinition[] = [
      ...
      ...visibleRelationships.map((relationship) => ({
        ...
        classes: evidenceIds.has(relationship.startId) && evidenceIds.has(relationship.endId) ? "evidence" : "context",
      })),
    ];
