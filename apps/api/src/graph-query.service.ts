...
import type { GraphNode, GraphRel, GraphSearchResponse, GraphStatsResponse, NeighborsResponse, QueryRequest, QueryResponse } from "@devloop/shared";
import { NodeLabelSchema, QueryRequestSchema, GraphSearchQuerySchema, NeighborsQuerySchema, RelationshipTypeSchema } from "@devloop/shared";
...
const ANCHOR_LABEL_QUOTAS: Partial<Record<GraphNode["label"], { min?: number; max?: number }>> = {
  Task: { max: 5 },
  Wiki: { min: 2, max: 3 },
  Concept: { max: 2 },
};
