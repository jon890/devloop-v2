...
test('강화 키로 사전 Concept을 canonical 노드 하나로 병합한다', () => {
  const aliasMap = buildConceptAliasMap([
    { canonical: 'OCR.API', kind: 'component', aliases: [] },
  ]);
...
test('부당 병합 denylist는 API 경로와 일반 이름을 분리한다', () => {
  const aliasMap = buildConceptAliasMap([
    { canonical: 'analysis', kind: 'code-ref', aliases: [] },
  ]);
  const graph = normalizeGraph(
    [
      {
...
test('사전 밖 Concept에도 부당 병합 denylist를 적용한다', () => {
  ...
  assert.deepEqual(
    graph.nodes.map((node) => node.key).sort(),
    ['*.cloud.toast.com', '/analysis', 'analysis', 'cloud.toast.com'],
  );
});
...
test('사전 밖 Concept은 관계 참조가 가장 많은 표기로 병합하고 원래 끝점도 해석한다', () => {
  ...
  assert.ok(
    graph.relationships.every(
      (relationship) => relationship.endKey === 'fastapi',
    ),
  );
...
test('태그 alias는 별도 canonical 없이 같은 Concept 관계 끝점으로 해석한다', () => {
  ...
  assert.equal(
    graph.nodes.filter((node) => node.label === 'Concept').length,
    1,
  );
  assert.ok(
    graph.relationships.every(
      (relationship) => relationship.endKey === 'Document AI',
    ),
  );
  assert.equal(graph.skippedRelationships.count, 0);
});
...
test('LLM Concept의 사전 매칭 여부와 출처를 독립적으로 기록한다', () => {
  const aliasMap = buildConceptAliasMap([
    { canonical: 'Request-Key', kind: 'tech', aliases: [] },
  ]);
  const graph = normalizeGraph(
    [
      {
  ...
  const unmatched = graph.nodes.find(
    (node) => node.key === 'unlisted.concept',
  );
