...
test('원본 계약대로 순차 수집하고 일시 실패를 3회 재시도한다', async () => {
  ...
  const fixtures = new Map<string, FixtureHandler>([
    ...
    [
      'post comment list sample 2 --json',
      async () => [{ id: 'c2', creator: { organizationMemberId: 'm2' } }],
    ],
    ['wiki pages sample --json', async () => [{ id: 'w1', root: true }]],
    [
      'wiki page get sample w1 --json',
      async () => ({ id: 'w1', creator: { member: { organizationMemberId: 'm1' } } }),
    ],
    ['wiki pages sample --parent w1 --json', async () => [{ id: 'w2' }]],
    [
      'wiki page get sample w2 --json',
      async () => ({ id: 'w2', creator: { member: { organizationMemberId: 'm2' } } }),
    ],
...
test('이미 저장된 파일은 다시 요청하지 않고 재개한다', async () => {
  ...
  const executor = new FixtureExecutor(
    new Map([['wiki pages sample --json', async () => [{ id: 'w1', root: true }]]]),
  );
