 ...
 */
export function findTaskReferences(text: string, sourceKey: string): TaskReference[] {
  const references: TaskReference[] = [];
  for (const match of text.matchAll(TASK_REFERENCE_PATTERN)) {
    const [matched, project, number] = match;
    if (!/[A-Za-z]/.test(project)) continue;
    if (NON_PROJECT_REFERENCE_KEYS.has(project.toLowerCase())) continue;
    ...
    references.push({ project, number });
  }
  return references;
}
